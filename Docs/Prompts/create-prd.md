...
1.  **Receive Initial Prompt:** The user provides a brief description or request for a new milestone in developing their first video game in Unreal Engine 5.7. If the user describes more than one feature or functionality, try to generate a high level PRD first if needed.
...
4.  **Save PRD:** Save the generated document as `[n]-prd-[feature-name].md`, a high level PRD as generated document as `[n]-prd-[project-phase-name].md` inside the `/tasks` directory. (Where `n` is a zero-padded 4-digit sequence starting from 0001, e.g., `0001-prd-user-authentication.md`, `0002-prd-dashboard.md`, etc.)
...
5.  **Security Requirements:** List all neccessary steps to make the feature as secure as possible, following cybersecurity best practices.
...
4. After providing the PRD, also provide an import ready CSV file to create GitHub issue for this milestone
